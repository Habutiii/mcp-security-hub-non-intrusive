...
async def query_bloodhound(query: str):
    """Run one of this module's fixed, read-only Cypher queries.

    This helper is intentionally not registered as an MCP tool. Every caller
    below provides a static MATCH/RETURN query; MCP clients cannot supply Cypher.
    """
    databases = ["neo4j", "bloodhound"]
    last_error = None
    ...
    for db in databases:
        try:
            with driver.session(database=db, default_access_mode=READ_ACCESS) as session:
                ...
                return {"success": True, "data": [record.data() for record in result]}
        except Exception as error:
            last_error = error
            logger.debug("Fixed query failed on database '%s': %s", db, error)
